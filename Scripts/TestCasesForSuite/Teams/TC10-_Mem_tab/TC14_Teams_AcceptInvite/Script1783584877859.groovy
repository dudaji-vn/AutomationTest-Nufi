...
/**
 * TC14: Teams - Accept Invitation as New Member
 * 
 * Test Flow:
...
WebUI.comment('=== TC14: Teams - Accept Invitation as New Member ===')

// Get team name and email from TC12 via GlobalVariable
// These should be set in TC12 before running TC14
...
try {
    ...
    if (!currentUrl.contains('/login')) {
        ...
        WebUI.takeScreenshot('TC14_Registration_StillOnRegisterPage.png')
    ...
    if (!hasInvitation) {
        ...
        WebUI.takeScreenshot('TC14_NoInvitation_Found.png')
        ...
        if (teamAlreadyExists) {
            ...
            WebUI.comment('✓ TC14 PASSED - Team already in member list')
            return
        }
    ...
    if (!teamFound) {
        ...
        if (teamFoundAlt) {
            ...
            WebUI.takeScreenshot('TC14_TeamNotFound_AfterAccept.png')
    ...
    WebUI.takeScreenshot('TC14_Teams_AcceptInvite_Success.png')
    WebUI.comment('=== TC14 PASSED ===')

} catch (Exception e) {
    WebUI.comment('=== TC14 FAILED: ' + e.getMessage() + ' ===')
    WebUI.takeScreenshot('TC14_Teams_AcceptInvite_Error.png')
    throw e
} finally {
