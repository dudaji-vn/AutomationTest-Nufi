...
/**
 * TC07_Parameters_Top_P_Input_Field
 * 
...
try {
    ...
    WebUI.takeScreenshot('TC07_TopP_Default.png')
    ...
    for (String[] pair : validValues) {
        ...
        WebUI.takeScreenshot("TC07_TopP_Valid_${pair[0].replace('.', '_')}.png")
    }
    ...
    WebUI.takeScreenshot('TC07_TopP_Clamp_Max.png')
    ...
    WebUI.takeScreenshot('TC07_TopP_Clamp_Min.png')
    ...
    try {
        ...
        if (finalNum.compareTo(min) >= 0 && finalNum.compareTo(max) <= 0) {
            WebUI.comment('✓ TC07 PASSED - Top P input field works correctly')
        } else {
