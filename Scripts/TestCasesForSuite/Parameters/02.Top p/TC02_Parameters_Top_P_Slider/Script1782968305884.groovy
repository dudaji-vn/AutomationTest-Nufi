...
WebUI.comment('=== TC02: Top P - Comprehensive Test ===')
...
try {
    ...
    WebUI.takeScreenshot('TC02_TopP_Min.png')
    ...
    WebUI.takeScreenshot('TC02_TopP_Max.png')
    ...
    for (String[] pair : validValues) {
        ...
        WebUI.takeScreenshot("TC02_TopP_Valid_${pair[0].replace('.', '_')}.png")
    }
    ...
    WebUI.takeScreenshot('TC02_TopP_Clamp_Max.png')
    
    // Clamp min: -0.5 → 0.0
    WebUI.comment('--- Testing clamp min (-0.5 → 0.0) ---')
    setValueAndBlur("-0.5")
    verifyNumericValue("0.0", "Below min test")
    WebUI.takeScreenshot('TC02_TopP_Clamp_Min.png')
    ...
    for (String invalid : invalidInputs) {
        ...
        WebUI.takeScreenshot("TC02_TopP_Invalid_" + invalid + ".png")
    ...
    WebUI.takeScreenshot('TC02_TopP_DoubleClick_Reset.png')
    ...
    try {
        ...
        WebUI.comment('TC02 PASSED - Top P parameter test completed successfully')
    } catch (Exception e) {
    ...
    WebUI.takeScreenshot('TC02_TopP_Complete.png')
    WebUI.comment('=== TC02 Completed ===')

} catch (Exception e) {
    WebUI.comment('TC02 FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC02_TopP_Error.png')
