...
/**
 * TC24B_Parameters_Reasoning_Effort_None
 * Verify dragging Reasoning Effort slider to None
 * 
 * Test Flow:
 * 1. Open Parameters panel
 * 2. Get Reasoning Effort slider
 * 3. Drag slider to None and verify
 */

WebUI.comment('=== TC24B: Reasoning Effort - Drag to None ===')
...
try {
    ...
    // Step size for 7 values (Auto, None, Minimal, Low, Medium, High, Extra High)
    double stepSize = sliderWidth / 6

    // ============================================================
    // STEP 4: DRAG TO NONE
    // ============================================================
    WebUI.comment('Step 4: Dragging slider to None...')
    
    int targetPosition = (int)(1 * stepSize) // Position 16.67%
    ...
    WebUI.comment('After dragging to None: ' + actualValue)
    
    if (actualValue == 'None') {
        WebUI.comment('TC24B PASSED - Slider moved to None successfully')
    } else {
        ...
        if (actualValue == 'None') {
            WebUI.comment('TC24B PASSED - Slider moved to None on second attempt')
        } else {
            WebUI.comment('FAILED: Could not drag slider to None. Current value: ' + actualValue)
            KeywordUtil.markFailed("FAILED: Could not drag slider to None. Current value: " + actualValue)
        }
    }

    WebUI.takeScreenshot('TC24B_ReasoningEffort_None.png')
    WebUI.comment('=== TC24B Completed ===')

} catch (Exception e) {
    WebUI.comment('TC24B FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC24B_ReasoningEffort_Error.png')
