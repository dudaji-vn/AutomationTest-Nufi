...
/**
 * TC24C_Parameters_Reasoning_Effort_Minimal
 * Verify dragging Reasoning Effort slider to Minimal
 * 
 * Test Flow:
 * 1. Open Parameters panel
 * 2. Get Reasoning Effort slider
 * 3. Drag slider to Minimal and verify
 */

WebUI.comment('=== TC24C: Reasoning Effort - Drag to Minimal ===')
...
try {
    ...
    // ============================================================
    // STEP 4: DRAG TO MINIMAL
    // ============================================================
    WebUI.comment('Step 4: Dragging slider to Minimal...')
    
    int targetPosition = (int)(2 * stepSize) // Position 33.33%
    ...
    WebUI.comment('After dragging to Minimal: ' + actualValue)
    
    if (actualValue == 'Minimal') {
        WebUI.comment('TC24C PASSED - Slider moved to Minimal successfully')
    } else {
        ...
        if (actualValue == 'Minimal') {
            WebUI.comment('TC24C PASSED - Slider moved to Minimal on second attempt')
        } else {
            WebUI.comment('FAILED: Could not drag slider to Minimal. Current value: ' + actualValue)
            KeywordUtil.markFailed("FAILED: Could not drag slider to Minimal. Current value: " + actualValue)
        }
    }

    WebUI.takeScreenshot('TC24C_ReasoningEffort_Minimal.png')
    WebUI.comment('=== TC24C Completed ===')

} catch (Exception e) {
    WebUI.comment('TC24C FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC24C_ReasoningEffort_Error.png')
