...
/**
 * TC03_Parameters_Frequency_Penalty_Test
 * 
 * Comprehensive Frequency Penalty parameter test covering:
 * 1. Min value (-2.0) - Slider test
 * 2. Max value (2.0) - Slider test
 * 3. Input field valid/invalid values
 * 4. Double click reset to default (0.0)
 */

WebUI.comment('=== TC03: Frequency Penalty - Comprehensive Test ===')
...
try {
    ...
    // ============================================================
    // STEP 2: GET FREQUENCY PENALTY SLIDER AND INPUT
    // ============================================================
    WebUI.comment('Step 2: Getting Frequency Penalty slider and input...')
    
    TestObject sliderThumb = new TestObject('freqPenaltySliderThumb')
    ...
    WebUI.comment('Frequency Penalty slider and input found')
    ...
    def getSliderWidth = {
        String script = """
            var element = document.evaluate(
                "//span[contains(@id,'frequency_penalty') or contains(@id,'freq-penalty') or contains(@id,'frequencyPenalty')]",
                document,
                null,
    ...
    // ============================================================
    // TEST 1: MIN VALUE (-2.0)
    // ============================================================
    WebUI.comment('=== TEST 1: Min Value (-2.0) ===')
    
    double actualMin = dragSliderTo(-2.0)
    WebUI.comment('Frequency Penalty after drag to min: ' + actualMin)
    
    if (Math.abs(actualMin - expectedMin) <= 0.05) {
        WebUI.comment('PASSED - Frequency Penalty set to -2.0')
    } else if (actualMin <= -1.8) {
        WebUI.comment('WARNING: Frequency Penalty set to near min: ' + actualMin)
    } else {
        WebUI.comment('WARNING: Frequency Penalty is ' + actualMin + ' instead of -2.0')
    }
    WebUI.takeScreenshot('TC03_FreqPenalty_Min.png')

    // ============================================================
    // TEST 2: MAX VALUE (2.0)
    // ============================================================
    WebUI.comment('=== TEST 2: Max Value (2.0) ===')
    
    double actualMax = dragSliderTo(2.0)
    WebUI.comment('Frequency Penalty after drag to max: ' + actualMax)
    
    if (Math.abs(actualMax - expectedMax) <= 0.05) {
        WebUI.comment('PASSED - Frequency Penalty set to 2.0')
    } else if (actualMax >= 1.8) {
        WebUI.comment('WARNING: Frequency Penalty set to near max: ' + actualMax)
    } else {
        WebUI.comment('WARNING: Frequency Penalty is ' + actualMax + ' instead of 2.0')
    }
    WebUI.takeScreenshot('TC03_FreqPenalty_Max.png')
    ...
    for (String[] pair : validValues) {
        ...
        WebUI.takeScreenshot("TC03_FreqPenalty_Valid_${pair[0].replace('-', 'neg').replace('.', '_')}.png")
    }
    ...
    WebUI.takeScreenshot('TC03_FreqPenalty_Clamp_Max.png')
    
    // Clamp min: -3.0 → -2.0
    WebUI.comment('--- Testing clamp min (-3.0 → -2.0) ---')
    setValueAndBlur("-3.0")
    verifyNumericValue("-2.0", "Below min test")
    WebUI.takeScreenshot('TC03_FreqPenalty_Clamp_Min.png')
    ...
    for (String invalid : invalidInputs) {
        ...
        WebUI.takeScreenshot("TC03_FreqPenalty_Invalid_" + invalid + ".png")
    ...
    try {
        ...
        if (actualNum.compareTo(expectedNum) == 0) {
            WebUI.comment('PASSED - Frequency Penalty reset to 0.0 successfully')
        } else {
            ...
            if (actualNum.compareTo(expectedNum) == 0) {
                WebUI.comment('PASSED - Frequency Penalty reset to 0.0 on second attempt')
            } else {
                WebUI.comment('FAILED: Frequency Penalty did not reset to 0.0. Actual: ' + afterReset)
                KeywordUtil.markFailed("FAILED: Frequency Penalty did not reset to 0.0. Actual: " + afterReset)
            }
        }
    } catch (Exception e) {
        KeywordUtil.markFailed("FAILED: Cannot convert value to number: " + afterReset)
    }
    WebUI.takeScreenshot('TC03_FreqPenalty_DoubleClick_Reset.png')
    ...
    WebUI.comment('Final Frequency Penalty value: ' + finalValue)
    
    try {
        BigDecimal finalNum = new BigDecimal(finalValue)
        WebUI.comment('TC03 PASSED - Frequency Penalty parameter test completed successfully')
    } catch (Exception e) {
    ...
    WebUI.takeScreenshot('TC03_FrequencyPenalty_Complete.png')
    WebUI.comment('=== TC03 Completed ===')

} catch (Exception e) {
    WebUI.comment('TC03 FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC03_FrequencyPenalty_Error.png')
