...
/**
 * TC03D: Parameters - Frequency Penalty Double Click Reset
 * Double click the slider thumb to reset to default value (0.0)
 */

WebUI.comment('=== TC03D: Frequency Penalty - Double Click Reset ===')
...
try {
    ...
    WebUI.comment('Step 2: Getting Frequency Penalty slider...')
    ...
    WebUI.comment('Frequency Penalty slider found')

    // === DRAG SLIDER TO DIFFERENT VALUE ===
    WebUI.comment('Step 3: Dragging slider to a different value...')
    WebUI.click(sliderThumb)  // Focus on thumb first
    WebUI.delay(0.5)
    ...
    WebUI.comment('Frequency Penalty after drag: ' + valueAfterDrag)
    ...
    WebUI.comment('Frequency Penalty after double click: ' + valueAfterDoubleClick)
    ...
    try {
        ...
        if (actualNum.compareTo(expectedDefault) == 0) {
            WebUI.comment('Frequency Penalty successfully reset to default value (0.0)')
            isResetSuccess = true
        }
    } catch (Exception e) {
    ...
    if (!isResetSuccess) {
        WebUI.comment('Reset failed on first attempt. Trying again...')
        WebUI.doubleClick(sliderThumb)
        WebUI.delay(1.5)
        ...
        try {
            ...
            if (actualNum.compareTo(new BigDecimal("0.0")) == 0) {
                WebUI.comment('Frequency Penalty reset to default on second attempt')
                isResetSuccess = true
            }
        } catch (Exception e) {}
    }
    ...
    if (!isResetSuccess) {
        KeywordUtil.markFailed("FAILED: Frequency Penalty did not reset to 0.0. Actual: " + valueAfterDoubleClick)
    }

    WebUI.takeScreenshot('TC12_FrequencyPenalty_DoubleClick_Reset.png')
    WebUI.comment('=== TC12 Completed Successfully ===')

} catch (Exception e) {
    WebUI.comment('TC12 FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC12_FrequencyPenalty_DoubleClick_Error.png')
