...
/**
 * TC27D_Parameters_Verbosity_High
 * Verify dragging Verbosity slider to High
 * 
 * Test Flow:
...
WebUI.comment('=== TC27D: Verbosity - Drag to High ===')
...
try {
    ...
    // STEP 4: DRAG TO HIGH
    // ============================================================
    WebUI.comment('Step 4: Dragging slider to High...')
    
    int targetPosition = (int)(3 * stepSize)
    ...
    WebUI.comment('After dragging to High: ' + actualValue)
    
    if (actualValue == 'High') {
        WebUI.comment('TC27D PASSED - Slider moved to High successfully')
    } else {
        ...
        if (actualValue == 'High') {
            WebUI.comment('TC27D PASSED - Slider moved to High on second attempt')
        } else {
            WebUI.comment('FAILED: Could not drag slider to High. Current value: ' + actualValue)
            KeywordUtil.markFailed("FAILED: Could not drag slider to High. Current value: " + actualValue)
        }
    }

    WebUI.takeScreenshot('TC27D_Verbosity_High.png')
    WebUI.comment('=== TC27D Completed ===')

} catch (Exception e) {
    WebUI.comment('TC27D FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC27D_Verbosity_Error.png')
