...
/**
 * TC27C_Parameters_Verbosity_Medium
 * Verify dragging Verbosity slider to Medium
 * 
 * Test Flow:
...
WebUI.comment('=== TC27C: Verbosity - Drag to Medium ===')
...
try {
    ...
    int targetPosition = (int)(2 * stepSize)
    ...
    if (actualValue == 'Medium') {
        WebUI.comment('TC27C PASSED - Slider moved to Medium successfully')
    } else {
        ...
        if (actualValue == 'Medium') {
            WebUI.comment('TC27C PASSED - Slider moved to Medium on second attempt')
        } else {
    ...
    WebUI.takeScreenshot('TC27C_Verbosity_Medium.png')
    WebUI.comment('=== TC27C Completed ===')

} catch (Exception e) {
    WebUI.comment('TC27C FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC27C_Verbosity_Error.png')
