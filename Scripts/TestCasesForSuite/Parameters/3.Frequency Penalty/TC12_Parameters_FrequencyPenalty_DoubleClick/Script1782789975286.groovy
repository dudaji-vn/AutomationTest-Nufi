...
try {
    ...
    double defaultValue = 0.0 // Frequency Penalty default là 0.0
    String currentValue = WebUI.getAttribute(sliderThumb, 'aria-valuenow')
    WebUI.comment('Current Frequency Penalty: ' + currentValue)
    ...
    if (Math.abs(actualValue - defaultValue) <= 0.05) {
        WebUI.comment('✅ Frequency Penalty reset to default (0.0) successfully')
    } else {
        WebUI.comment('⚠ Frequency Penalty not reset to default: ' + actualValue)
        ...
        WebUI.comment('Frequency Penalty after second double click: ' + valueAfterDoubleClick)
    ...
    WebUI.comment('✅ TC12 PASSED - Frequency Penalty Double Click Reset test')
