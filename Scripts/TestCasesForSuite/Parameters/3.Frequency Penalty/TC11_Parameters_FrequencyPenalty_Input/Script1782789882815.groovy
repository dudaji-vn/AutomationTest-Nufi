...
/**
 * TC11: Parameters - Frequency Penalty Input Field
 * Verify direct input into Frequency Penalty field (-2.0 - 2.0)
 */

WebUI.comment('=== TC11: Frequency Penalty - Input Field Test ===')
...
try {
    ...
    // === TEST FREQUENCY PENALTY INPUT FIELD ===
    WebUI.comment('Step 2: Testing Frequency Penalty input field...')
    
    // Use Object Repository for the input field
    ...
    // List of valid test values (-2.0 - 2.0)
    ...
    for (String value : testValues) {
        ...
        WebUI.takeScreenshot("TC11_FrequencyPenalty_Input_${value.replace('.', '_').replace('-', 'neg')}.png")
    }
    ...
    // Check invalid input below minimum (-3.0 < -2.0)
    ...
    WebUI.takeScreenshot('TC11_FrequencyPenalty_Input_Final.png')

    WebUI.comment('=== TC11 Completed - Frequency Penalty Input Field Test ===')
    WebUI.comment('✅ TC11 PASSED')

} catch (Exception e) {
    WebUI.comment('❌ TC11 FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC11_FrequencyPenalty_Input_Error.png')
