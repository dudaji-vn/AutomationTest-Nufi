...
/**
 * TC26B_Parameters_Reasoning_Summary_Auto
 * Verify dragging Reasoning Summary slider to Auto
 * 
 * Test Flow:
 * 1. Open Parameters panel
 * 2. Get Reasoning Summary slider
 * 3. Drag slider to Auto and verify
 */

WebUI.comment('=== TC26B: Reasoning Summary - Drag to Auto ===')
...
try {
    ...
    // STEP 4: DRAG TO AUTO
    // ============================================================
    WebUI.comment('Step 4: Dragging slider to Auto...')
    ...
    WebUI.comment('After dragging to Auto: ' + actualValue)
    
    if (actualValue == 'Auto') {
        WebUI.comment('TC26B PASSED - Slider moved to Auto successfully')
    } else {
        ...
        if (actualValue == 'Auto') {
            WebUI.comment('TC26B PASSED - Slider moved to Auto on second attempt')
        } else {
            WebUI.comment('FAILED: Could not drag slider to Auto. Current value: ' + actualValue)
            KeywordUtil.markFailed("FAILED: Could not drag slider to Auto. Current value: " + actualValue)
        }
    }

    WebUI.takeScreenshot('TC26B_ReasoningSummary_Auto.png')
    WebUI.comment('=== TC26B Completed ===')

} catch (Exception e) {
    WebUI.comment('TC26B FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC26B_ReasoningSummary_Error.png')
