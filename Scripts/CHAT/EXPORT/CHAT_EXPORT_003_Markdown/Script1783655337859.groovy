 ...
 * 11. Click Export
 * 12. Verify file downloaded with .md extension
 */

WebUI.comment('=== TC36: Export - Markdown (.md) ===')
...
try {
    ...
    // Step 7: Select Type = "markdown (.md)"
    WebUI.comment('Step 7: Selecting Type = "markdown (.md)"...')
    WebUI.click(findTestObject('Object Repository/Core Chat/Share_chat/Export/dropdown_combobox'))
    WebUI.delay(1)
    WebUI.click(findTestObject('Object Repository/Core Chat/Share_chat/Export/item_listbox_type/opt_markdown (.md)'))
    WebUI.comment('Type changed to markdown (.md)')
    WebUI.delay(1)
    ...
    // Step 12: Verify file downloaded with .md extension
    WebUI.comment('Step 12: Verifying file downloaded...')
    String expectedFilename = filename + '.md'
    WebUI.comment('Expected filename: ' + expectedFilename)
    
    WebUI.takeScreenshot('TC36_Export_Markdown_Success.png')
    WebUI.comment('Export markdown completed')

    // Step 13: Close browser
    WebUI.comment('Step 13: Closing browser...')
    CustomKeywords.'keywords.ChatKeywords.closeBrowser'()

    WebUI.comment('TC36 PASSED')

} catch (Exception e) {
    WebUI.comment('TC36 FAILED: ' + e.getMessage())
    WebUI.takeScreenshot('TC36_Export_Markdown_Error.png')
    CustomKeywords.'keywords.ChatKeywords.closeBrowser'()
    throw e
}