...
/**
 * TC20: Dislike Response - Poor style or tone
 * 
 * Test Flow:
 * 1. Open browser
 * 2. Login
 ...
 * 7. Select "Poor style or tone" reason
 * 8. Verify dislike success
 */

WebUI.comment('=== TC20: Dislike Response (Poor style or tone) ===')
...
try {
    ...
    WebUI.comment('Step 7: Selecting "Poor style or tone" reason...')
    ...
    TestObject reasonOption = findTestObject('Object Repository/Core Chat/Action/Needs improvement/button_Poor style or tone')
    WebUI.waitForElementVisible(reasonOption, 5)
    WebUI.click(reasonOption)
    WebUI.comment('"Poor style or tone" reason selected')

    WebUI.comment('Step 8: Verifying popup closed...')
    WebUI.waitForElementNotVisible(popup, 5)
    WebUI.comment('Popup closed successfully')
    WebUI.takeScreenshot('TC20_Dislike_PoorStyle_Success.png')
    ...
    WebUI.takeScreenshot('TC20_Dislike_PoorStyle_Error.png')
    CustomKeywords.'keywords.ChatKeywords.closeBrowser'()
    throw e
}