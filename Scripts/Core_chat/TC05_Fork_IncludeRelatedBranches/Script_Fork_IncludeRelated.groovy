 ...
 * 7. Click "Include related branches" option
...
try {
    ...
    WebUI.waitForElementVisible(forkMenuButton, 10)
    WebUI.click(forkMenuButton)
    WebUI.comment('Fork menu button clicked')

    // Step 7: Click "Include related branches" option
    WebUI.comment('Step 7: Clicking "Include related branches" option...')
    
    // Use Object Repository if available, otherwise use dynamic XPath
    TestObject includeRelatedOption
    try {
        includeRelatedOption = findTestObject('Object Repository/Core Chat/Page_T C Gio/Fork/Opt_Include related branches')
        WebUI.comment('Using Object Repository for Include related branches option')
    } catch (Exception e) {
        WebUI.comment('Object Repository not found, using dynamic XPath')
        includeRelatedOption = new TestObject('dynamic_include_related_option')
        includeRelatedOption.addProperty('xpath', ConditionType.EQUALS, 
            "//button[@aria-label='Include related branches']")
    }
    
    WebUI.waitForElementVisible(includeRelatedOption, 10)
    ...
    WebUI.waitForElementVisible(forkSuccessMessage, 10)
